...
select * from Orders_detail where order_id in (select order_id from Orders where user_mail='Dao Trong Duy')
select D.order_id,D.product_id,D.amount_product,D.price_product from Orders as O join Orders_detail as D on O.order_id = D.order_id where user_mail='Dao Trong Duy'